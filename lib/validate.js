const { splitArgs, parseArg } = require('./parse');
...
/**
 * Resolve a positional arg spec by parsing additional information from `name` field
 * and ensuring the `optional` property is used as a fallback for `required`
 * @param {Object} obj - a positional argument spec
 * @returns {Object}
 */
function resolveArg(obj) {
    const { name, variadic, required } = parseArg(obj.name),
        arg = { variadic, ...obj, name };
    // ensure `required` field is a boolean
    ensureRequiredField(arg, required);
    return arg;
}
        ...
        if (Array.isArray(arg)) {
            if (arg.some((a) => !(typeof a === 'string' || (isObject(a) && typeof a.name === 'string')))) {
                ...
                opt.arg = arg.flatMap((a) => {
                    if (typeof a !== 'string') return [resolveArg(a)];
                    try {
                        return splitArgs(a).map((s) => parseArg(s));
                    } catch ({ message }) {
            ...
            if (typeof arg.name !== 'string') {
                ...
                opt.arg = resolveArg(arg);
            }
        } else if (typeof arg !== 'string') {
            errors.push(`Invalid option arg spec: improper value provided for option '${name}'`);
        } else {
            try {
                opt.arg = splitArgs(arg).map((s) => parseArg(s));
            } catch ({ message }) {
                errors.push(`Invalid option arg spec: ${message}`);
            }
        }
    });
    // validate `dependsOn` field for each option
    opts.forEach((opt) => {
        const { name, dependsOn } = opt;
        if (!dependsOn) return;
        // ensure `dependsOn` prop is a string
        if (typeof dependsOn !== 'string') {
            errors.push(`Invalid dependsOn field: type of reference specified for option '${name}' is not a string`);
            return;
        }
        // remove dash prefix to normalize reference
        let ref = dependsOn.replace(/^-+/, '');
        // ensure `dependsOn` prop is a valid option name
        if (!ref.length) {
            errors.push(`Invalid dependsOn field: '${dependsOn}' is not a valid option reference`);
            return;
        }
        // check for an alias
        if (Object.hasOwnProperty.call(aliasNames, ref)) ref = aliasNames[ref];
        // ensure `dependsOn` points to an existing option
        if (!optionNames.includes(ref)) {
            errors.push(`Invalid dependsOn reference: option '${dependsOn}' is not specified`);
            return;
        }
        // if `dependsOn` points to itself, silently remove it
        if (name === ref) {
            opt.dependsOn = undefined;
            return;
        }
        opt.dependsOn = ref;
        // ensure option does not require a conflicting ref
        if (conflictMap[name].includes(ref) || conflictMap[ref].includes(name)) {
            errors.push(`Contradictory reference: option '${name}' both conflicts with and depends on '${ref}'`);
        }
    });
    // check for circular `dependsOn` relationships
    opts
        .map(({ name, dependsOn }) => (dependsOn && [name, dependsOn]))
        .filter(Boolean)
        .forEach(([name, req], i, pairs) => {
            if (pairs.slice(i + 1).some(([n, r]) => (req === n && name === r))) {
                errors.push(`Circular dependency: options '${name}' and '${req}' depend on each other`);
            }
        });
    return [opts, errors];
}
