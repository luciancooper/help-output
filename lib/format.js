...
module.exports = class Formatter {
    ...
    stringifyArg(arg) {
        ...
        const { name, required = true, variadic = false } = arg;
        let str = /^<.+>$/.test(name) ? name.slice(1, -1) : name;
        if (variadic) str = `${str}...`;
        // add optional / required brackets
        str = required ? `<${str}>` : `[${str}]`;
        // colorize arg
    ...
    positionalRows(args) {
        return args.map(({ name, description, ...opts }) => {
            let str = /^<.+>$/.test(name) ? name.slice(1, -1) : name;
            if (opts.variadic) str = `${str}...`;
            // add optional / required brackets
            str = opts.required ? `<${str}>` : `[${str}]`;
            if (this.color) str = this.styles.positional(str);
