...
module.exports = (schema, {
    ...
    const { args, options } = schema,
        ...
        argrows = args.map(({ name, description, ...opts }) => {
            let str = name;
        ...
        optrows = options.map(({ name, arg, description }) => [
            styles.name(name),
            arg && styles.arg(formatArgument(arg)),
            description,
        ]),
        // allocate column widths
        [lcol, rcol] = allocateColumnWidths([...argrows, ...optrows], width - indent, spacing);
    // format each output section
    return [[
        'USAGE:',
        // format usage description
        formatUsage(cmdname, schema, lcol + rcol + (rcol ? spacing : 0)),
    ], [
        'ARGUMENTS:',
        // align argument columns
        alignColumnContent(argrows, lcol, rcol, spacing),
    ], [
        'OPTIONS:',
        // align option columns
        alignColumnContent(optrows, lcol, rcol, spacing),
    ]].map(([title, content]) => (
        content && [
            styles.title(title),
