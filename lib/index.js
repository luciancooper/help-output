const { stringWidth, wordWrap } = require('tty-strings'),
    validate = require('./validate'),
    optionsUsage = require('./options'),
    Formatter = require('./format'),
    formatUsage = require('./usage'),
    { terminalWidth, scriptName } = require('./utils');
...
function allocateColumnWidths(rows, width, spacing) {
    ...
    const argExp = Math.max(...rows.map(([name, arg = '']) => stringWidth(name + (arg && ` ${arg}`)))),
        // max description column width
        descMax = Math.max(...rows.map(([,, description = '']) => stringWidth(description)));
    ...
    const argClpse = Math.max(...rows.map(([name, arg]) => (
        arg ? Math.max(stringWidth(name) + 1, stringWidth(arg)) : stringWidth(name)
    )));
    ...
    const dscExp = Math.max(width - argExp - spacing, 0),
        ...
        [hExp, hClpse] = rows.map(([name, arg = '', description = '']) => {
            ...
            const wrapExp = wordWrap(description, dscExp).split('\n'),
                // wrap description to space available with args collapsed
                wrapClpse = wordWrap(description, dscClpse).split('\n');
            return [
                // row height with args expanded
                wrapExp.some((l) => stringWidth(l) > dscExp) ? NaN : wrapExp.length,
                // row height with args collapsed
                wrapClpse.some((l) => stringWidth(l) > dscClpse) ? NaN : Math.max(
                    stringWidth(name + (arg && ` ${arg}`)) > argClpse ? 2 : 1,
                    wrapClpse.length,
                ),
            ];
...
function alignColumnContent(rows, lcol, rcol, spacing) {
    ...
    return rows.flatMap(([name, arg = '', description = '']) => {
        ...
        const [nl, al] = [stringWidth(name), stringWidth(arg)],
            n = nl + (al && al + 1),
            id = (n > lcol)
        ...
        const wrapped = wordWrap(description, rcol).split('\n'),
        ...
        return Array(height).fill(' '.repeat(spacing)).map((gap, i) => (
            (id[i] ? id[i] + ' '.repeat(lcol - stringWidth(id[i])) : ' '.repeat(lcol))
...
function createOutputHead(name, config, width) {
    ...
    if (config.description) {
        desc = wordWrap(String(config.description), width);
        // check if any word in the description is longer than the available width
        if (desc.split('\n').some((l) => stringWidth(l) > width)) {
            desc = (width >= 40)
                // if width is sufficiantly large, then use hard wrap
                ? wordWrap(String(config.description), width, { hard: true })
...
module.exports = (config, {
    ...
    const name = config.name ? String(config.name) : scriptName(),
